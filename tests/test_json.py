...
from generate_dummyf.generate import generate_json
...
class JsonTestCase(unittest.TestCase):
    def test_invalid_file_size_data_type(self):
        ...
        self.assertRaises(InvalidInputDataTypeException, generate_json(file_size="1000"))
    ...
    def test_invalid_column_description_value_data_type(self):
        ...
        self.assertRaises(InvalidInputDataTypeException, generate_json(file_size=3824, column_description={"Roll No.": int}))

    def test_invalid_file_path(self):
        """Case 3: function is given invalid path for saving file"""
        self.assertRaises(InvalidPathException, generate_json(1000, save_file_path="bhsdajhbfsdj"))

    def test_invalid_file_size_value(self):
        """Case 4:  function is provided an invalid file size"""
        self.assertRaises(InvalidSizeException, generate_json(file_size=-1000))
    ...
    def test_invalid_column_description_value(self):
        ...
        self.assertRaises(InvalidColumnDescriptionValueException, generate_json(file_size=1000, column_description={"file": "object"}))

    def test_file_generation(self):
        """Case 6: if file is generated or not """
        expected_file_path = os.path.expanduser("~")
        generate_json(file_size=1000, save_file_path=expected_file_path)

        # Assert that the file exists --> if AssertionError is not raised, then the test is successful
        self.assertEqual(os.path.exists(f"{expected_file_path}/Dummy.json"), True, f"File {expected_file_path}/Dummy.json was not generated")
